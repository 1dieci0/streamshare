...
use crate::{
    client::session::ClientSession, media::{capture, encoder::{self, FFmpegEncoder}, frame::RawFrame, state::Media}, network::{self, stream::{receive_packet, send_packet}}, protocol::{command::{ClientPacket, ServerPacket}, video::VideoPacket}, server::Server, ui};
...
impl Client {
    ...
    pub async fn start(&self) -> anyhow::Result<()>{
        ...
        //let (encoder_tx, encoder_rx) = tokio::sync::mpsc::channel(128);
        ...
        capture::start_capture(self.media.clone(), raw_tx);
